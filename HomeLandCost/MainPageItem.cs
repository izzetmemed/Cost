...
{
    public partial class MainPageItem : Form
    {
        private CostItemDA ItemDA = new CostItemDA();
        ...
        public void RefreshDataMonth()
        {
            ...
            var allData = ItemDA.GetAll(x => x.DeletedId == 0 && x.AddTime.Month == DateTime.Now.Month);
        ...
        public void RefreshData()
        {
            ...
            var allData = ItemDA.GetAll(x => x.DeletedId == 0);
            List<object> dataList = new List<object>();
            foreach (var item in allData)
            {
                var data = new
        ...
        private void GridData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == GridData.Columns["Delete"].Index && e.RowIndex >= 0)
            {
                ...
                var data = ItemDA.Get(x => x.Id == selectedTodoId);
            ...
            {
                int selectedTodoId = (int)GridData.Rows[e.RowIndex].Cells["Id"].Value;
                Edit edit = new Edit(selectedTodoId, "Item");
                edit.Show();
                this.Hide();
            }
        }
        ...
        {
            string fileName = "HomeLandCostItem.docx";
            try
            {
